...
NPC_ISCharacterInfo = ISPanelJoypad:derive("NPC_ISCharacterInfo");
NPC_ISCharacterInfo.timerMultiplierAnim = 0;
...
function NPC_ISCharacterInfo:initialise()
	ISPanelJoypad.initialise(self);
end

function NPC_ISCharacterInfo:createChildren()
	self:setScrollChildren(true)
	self:addScrollBars()
end

function NPC_ISCharacterInfo:setVisible(visible)
...
function NPC_ISCharacterInfo:prerender()
	ISPanelJoypad.prerender(self)
	self:setStencilRect(0, 0, self.width, self.height)
end

function NPC_ISCharacterInfo:render()
	local tabHeight = self.y
	local maxHeight = getCore():getScreenHeight() - ISWindow.TitleBarHeight - tabHeight - 20
	local y = 10
	NPC_ISSkillProgressBar.updateAlpha() -- FIXME: do this once per frame, not for each player
	...
	NPC_ISCharacterInfo.timerMultiplierAnim = NPC_ISCharacterInfo.timerMultiplierAnim + ms;
	if NPC_ISCharacterInfo.timerMultiplierAnim <= 500 then
        NPC_ISCharacterInfo.animOffset = -1;
	elseif NPC_ISCharacterInfo.timerMultiplierAnim <= 1000 then
        NPC_ISCharacterInfo.animOffset = 0;
	elseif NPC_ISCharacterInfo.timerMultiplierAnim <= 1500 then
        NPC_ISCharacterInfo.animOffset = 15;
	elseif NPC_ISCharacterInfo.timerMultiplierAnim <= 2000 then
        NPC_ISCharacterInfo.animOffset = 30;
	else
		NPC_ISCharacterInfo.timerMultiplierAnim = 0;
	end
	...
	for _,parentPerk in ipairs(sorted) do
		...
		for ind, perk in ipairs(perkList) do
            ...
            if self.char:getXp():getMultiplier(perk:getType()) > 0 then
                ...
                if NPC_ISCharacterInfo.animOffset > -1 then
                    self:drawTexture(self.arrow, left + 20 + self.txtLen + NPC_ISCharacterInfo.animOffset, y, 1, 1, 1, 1);
                end
            end
			if not self.progressBarLoaded then
				local progressBar = NPC_ISSkillProgressBar:new(left + 20 + self.txtLen + 45, y + (rowHgt - progressHgt) / 2, 0, 0, self.char, perk);
...
function NPC_ISCharacterInfo:onMouseWheel(del)
	self:setYScroll(self:getYScroll() - del * 30)
	return true
end

function NPC_ISCharacterInfo:new (x, y, width, height, character)
	local o = {};
	...
	o:noBackground();
	o.txtLen = 0;
	o.perks = NPC_ISCharacterInfo.loadPerk(o);
    ...
    NPC_ISCharacterInfo.instance = o;
   return o;
end

NPC_ISCharacterInfo.loadPerk = function(self)
	local perks = {};
...
function NPC_ISCharacterInfo:ensureVisible()
...
function NPC_ISCharacterInfo:updateTooltipForJoypad()
	if self.joypadIndex and self.joypadIndex >= 1 and self.joypadIndex <= #self.progressBars then
...
function NPC_ISCharacterInfo:onGainJoypadFocus(joypadData)
    ISPanelJoypad.onGainJoypadFocus(self, joypadData);
	self.joypadIndex = nil
	self.barWithTooltip = nil
end

function NPC_ISCharacterInfo:onLoseJoypadFocus(joypadData)
    ISPanelJoypad.onLoseJoypadFocus(self, joypadData);
end

function NPC_ISCharacterInfo.onResolutionChange(oldw, oldh, neww, newh)
...
Events.OnResolutionChange.Add(NPC_ISCharacterInfo.onResolutionChange)
