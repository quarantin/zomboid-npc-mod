...
NPC_ISSkillProgressBar = ISPanel:derive("NPC_ISSkillProgressBar");

NPC_ISSkillProgressBar.alpha = 0.0;
NPC_ISSkillProgressBar.upAlpha = true;


function NPC_ISSkillProgressBar:initialise()
	ISPanel.initialise(self);
end

function NPC_ISSkillProgressBar:render()
	self:renderPerkRect();
...
function NPC_ISSkillProgressBar:onMouseUp(x, y)
	-- our selected lvl
	...
	if self.xp >= self.xpForLvl and self.level == lvlSelected and not self.perk:isPassiv() then
		...
		self.xpForLvl = NPC_ISSkillProgressBar.getXpForLvl(self.perk, self.level);
		-- reset the tooltip
		self.message = nil;
		if self.tooltip ~= nil then
...
function NPC_ISSkillProgressBar:updateTooltip(lvlSelected)
...
function NPC_ISSkillProgressBar:onMouseMove(dx, dy)
...
function NPC_ISSkillProgressBar:onMouseMoveOutside(dx, dy)
	self.message = nil;
	if self.tooltip ~= nil then
		self.tooltip:setVisible(false);
		self.tooltip:removeFromUIManager();
...
function NPC_ISSkillProgressBar:activate()
end

function NPC_ISSkillProgressBar:renderPerkRect()
	local x = 0;
	local y = 0;
	...
	self.xp = NPC_ISSkillProgressBar.getPerkXp(self);
...
function NPC_ISSkillProgressBar.updateAlpha()
	if NPC_ISSkillProgressBar.upAlpha then
		NPC_ISSkillProgressBar.alpha = NPC_ISSkillProgressBar.alpha + 0.1 * (30 / getPerformance():getUIRenderFPS());
		if NPC_ISSkillProgressBar.alpha > 1.0 then
			NPC_ISSkillProgressBar.alpha = 1.0;
			NPC_ISSkillProgressBar.upAlpha = false;
		end
	else
		NPC_ISSkillProgressBar.alpha = NPC_ISSkillProgressBar.alpha - 0.1 * (30 / getPerformance():getUIRenderFPS());
		if NPC_ISSkillProgressBar.alpha < 0 then
			NPC_ISSkillProgressBar.alpha = 0;
			NPC_ISSkillProgressBar.upAlpha = true;
		end
	end
end

function NPC_ISSkillProgressBar:new (x, y, width, height, character, perk)
	local o = {};
	...
	-- how much xp we need for the next lvl
	o.xpForLvl = NPC_ISSkillProgressBar.getXpForLvl(perk, o.level);
...
NPC_ISSkillProgressBar.getXpForLvl = function(perk, level)
	return perk:getXpForLevel(level + 1)
end

NPC_ISSkillProgressBar.getPreviousXpLvl = function(perk, level)
	if level == 0 then
		return 0;
	end
	level = level - 1;
...
NPC_ISSkillProgressBar.getPerkXp = function(self)
	return self.char:getXp():getXP(self.perk:getType()) - NPC_ISSkillProgressBar.getPreviousXpLvl(self.perk, self.level);
end
